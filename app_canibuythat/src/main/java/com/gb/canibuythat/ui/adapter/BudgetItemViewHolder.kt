...
class BudgetItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    val amountRepetitionView: TextView = itemView.findViewById(R.id.amount_repetition) as TextView
    ...
    fun bind(budgetItem: BudgetItem) {
        ...
        if (budgetItem.occurrenceCount == null) {
            ...
            if (periodType!!.strRes > 0) {
                ...
                amountRepetitionView!!.text = context.resources.getQuantityString(R.plurals.amount_per_period, budgetItem.periodMultiplier!!, Math.abs(budgetItem.amount!!), budgetItem.periodMultiplier, periodStr)
            }
        } else {
            amountRepetitionView!!.text = context.resources.getQuantityString(R.plurals.amount_times, budgetItem.occurrenceCount!!, Math.abs(budgetItem.amount!!), budgetItem.occurrenceCount)
        }
